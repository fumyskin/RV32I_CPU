...
entity FMAMemBuffer is
    ...
    port(
        ...
        abc_valid: out std_logic                                            -- signal flag to notify fma module stores valid data
...
architecture Behavioral of FMAMemBuffer is
    ...
    abc_input <= abc;  --assign abc signal to bit string coming in
    ...
    begin
        if (rst = '1') then
            current_state <= IDLE; -- go in idle state
            ...
            case (current_state) is
                ...
                when FILL =>
                    -- first verify that 
                    for i in 0 to FMA_NUM-1 loop
                        if valid_array(i).a_valid = '1' then
                            abc_reg(i).a <= input_array(i).a;
                        end if;

                        if valid_array(i).b_valid = '1' then
                            abc_reg(i).b <= input_array(i).b;
                        end if;
